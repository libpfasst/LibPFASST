module utils
 ...
 contains
  ...
  subroutine initial(L)
    ...
    class(zmkpair), pointer :: L_p
    integer :: Nmat
    L_p => cast_as_zmkpair(L)
    Nmat= L_p%dim
    
    call exact(L_p, 0.0_pfdp)
    select case(nprob)
    case (1)
       call init_toda(L_p%y,Nmat)
    case (2)
       call init_Facke(L_p%y,Nmat)
    case default
  ...
  subroutine exact(L, t)
    ...
    complex(pfdp),  intent(inout) :: L_array(:,:)
  ...
  subroutine compute_F_toda(L_array,B_array,Nmat,t,level)
    ...
    complex(pfdp), intent(inout) :: L_array(:,:), B_array(:,:)
    integer,intent(in) :: Nmat
    real(pfdp), intent(in) :: t
