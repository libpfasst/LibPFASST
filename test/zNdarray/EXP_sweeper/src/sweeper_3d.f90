...
! Exponential Sweeper for 3-D  examples
module pf_my_sweeper
  use pf_mod_dtype
  use pf_mod_zndarray
...
contains
  ...
  subroutine initialize_tmp(this)
    ...
    allocate(this%P_sweep(nx,nx,nx, this%nnodes - 1, 2),STAT=istat)
    if (istat .ne. 0)  call pf_stop(__FILE__,__LINE__,'Allocate failed ',istat)
    allocate(this%W_sweep(nx,nx,nx,this%nnodes, this%nnodes - 1),STAT=istat)
    if (istat .ne. 0)  call pf_stop(__FILE__,__LINE__,'Allocate failed ',istat)
  ...
  subroutine initWSweep(this,dt)
    ...
    allocate(p(nnodes+1,this%nx,this%nx,this%nx))
    ...
    do m=1,nnodes-1
      ...
      call phi_zmatrix3d(eta(m)*dt*this%fft_ops%opL,nnodes,p)            ! phi functions 0 to n
