...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine exact_realspace(t, yex)
    use probin, only: eq_type,lam1,lam2,nu, a,b, t00, kfreq,Lx,beta,ic_type
    real(pfdp), intent(in)  :: t
    ...
    case (1)  !  Advection diffusion
       if (ic_type .eq. 1) then
          call exact_ad_cos(t,yex,nu,[a,b],[kfreq,kfreq],[Lx,Lx])
       else
          call exact_ad_exp(t,yex,nu,[a,b],[Lx,Lx])
       end if
    case (2)  !  Burgers
       call exact_burg_sin(t,yex,nu,[Lx,Lx])
    case (4)  !  KdV
