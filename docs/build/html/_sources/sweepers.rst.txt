...
* ``amisdc``   : Asynchronous multi-implicit integrator for equations with an two implicit  and 1 explicit splitting
...
* ``imexQ_oc`` : Optimal control version of imex
* ``misdcQ_oc``: Optimal control version of misdc

For historical reasons, there are might be two versions of some of these sweepers, with a ``Q`` denoting the more modern version that can utilize DIRK style sweepers.  
