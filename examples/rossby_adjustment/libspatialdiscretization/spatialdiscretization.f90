...
! November 2, 2012
!
USE FVMParameters,           only : Nx, Ny, dim, ReadFVMParameter
...
CONTAINS
	...
	SUBROUTINE InitializeSpatialDiscretization(maxit, Nparareal_restarts, mpi_init_thread_flag, mpi_communicator, Nthreads, echo_on, dim)
		...
		CALL ReadFVMParameter(echo_on, dim)
		
		CALL InitializeFiniteVolumes(Nx, Ny,                                                                         Nthreads = Nthreads, mpi_init_thread = mpi_init_thread_flag, echo_on = echo_on)
