...
PRIVATE
PUBLIC :: GetRHS_inner, GetRHS_boundary, InitializeFiniteVolumes, FillinGhostcells, GhostLeft, GhostRight, GhostUp, GhostDown, GhostCorners, &
...
DOUBLE PRECISION, ALLOCATABLE, SAVE, DIMENSION(:,:,:,:) :: GhostLeft, GhostRight, GhostUp, GhostDown, GhostFluxLeft, GhostFluxRight, GhostFluxUp, GhostFluxDown

! Define buffers storing the horizontal cell and interface flux values
DOUBLE PRECISION, ALLOCATABLE, SAVE, DIMENSION(:,:,:,:) :: FluxInt_hor, FluxCell_hor, FluxInt_ver, FluxCell_ver
...
CONTAINS
	...
	SUBROUTINE GetRHS_inner(Q, order_advection, order_diffusion, RQ, dy, dx, nu)
		...
		CALL UpdateHorizontalFlux_inner(Q, dble(1.0) )
		CALL UpdateVerticalFlux_inner(Q,   dble(1.0) )
		
		!CALL UpdateHorizontalFlux_Upwind_inner(Q)
		!CALL UpdateVerticalFlux_Upwind_inner(Q)
		
		!! Compute flux divergence
		CALL GetFluxDivergence_inner_Upwind(RQ, dy, dx)		
		
		! Add contribution from diffusion
		!RQ = 0.0 ! set to zero to test effect of diffusion only
		CALL AddDiffusion(Q, RQ, dx, dy, nu, order_diffusion)
		
		CONTAINS
		
			SUBROUTINE UpdateHorizontalFlux_Upwind_inner(Q)
				DOUBLE PRECISION, DIMENSION(:,:,:), INTENT(IN) :: Q
				...
				DO i=2,Nx
					DO j=1,Ny
						...
						! For linear advection, set interface value to constant value manually
						Uvel = 1.0
						
						! Compute interface velocity by simple linear interpolation (Burger's equation)
						!Uvel = weight*( Q(1,j,i-1) + Q(1,j,i) )
						
						! Then compute flux at interface
						FluxInt_hor(1,j,i,thread_nr) = Uvel*weight*( Q(1,j,i) + Q(1,j,i-1) ) - weight*ABS(Uvel)*( Q(1,j,i) - Q(1,j,i-1) )
						
					END DO
				END DO
				
			END SUBROUTINE UpdateHorizontalFlux_Upwind_inner
			
			SUBROUTINE UpdateVerticalFlux_Upwind_inner(Q)
				DOUBLE PRECISION, DIMENSION(:,:,:), INTENT(IN) :: Q

				INTEGER          :: Nx, Ny, i, j, thread_nr
				DOUBLE PRECISION :: Vvel, weight = 1.0/2.0
				...
				DO i=1,Nx
					DO j=2,Ny
						...
						! Burger's equation
						!Vvel = weight*( Q(1,j-1,i) + Q(1,j,i) )
						
						FluxInt_ver(1,j,i,thread_nr) = Vvel*weight*( Q(1,j,i) + Q(1,j-1,i) ) - weight*ABS(Vvel)*( Q(1,j,i) - Q(1,j-1,i) )
						
					END DO
				END DO
				
			END SUBROUTINE UpdateVerticalFlux_Upwind_inner
	...
	END SUBROUTINE GetRHS_inner
	
	SUBROUTINE GetRHS_boundary(Q, order_advection, order_diffusion, RQ, dy, dx, nu)
		...
		! Now update interface values of horizontal flux
		CALL UpdateHorizontalFlux_boundary(Q, dble(1.0) )
		CALL UpdateVerticalFlux_boundary(Q,   dble(1.0) )
		
		!CALL UpdateHorizontalFlux_Upwind_boundary(Q)
		!CALL UpdateVerticalFlux_Upwind_boundary(Q)
		
		! Compute flux divergence
		CALL GetFluxDivergence_boundary_WENO5(RQ, dy, dx)
		!CALL GetFluxDivergence_boundary_Upwind(RQ, dy, dx)
		...
		CONTAINS
			...
			SUBROUTINE UpdateHorizontalFlux_Upwind_boundary(Q)
				DOUBLE PRECISION, DIMENSION(:,:,:), INTENT(IN) :: Q
				...
				DO j=1,Ny
					...
					! Burger's equation
					!Uvel = weight*( GhostLeft(1,1,j,thread_nr) + Q(1,j,i) )
					
					FluxInt_hor(1,j,i,thread_nr) = Uvel*weight*( GhostLeft(1,j,1,thread_nr) + Q(1,j,i) ) - weight*ABS(Uvel)*( Q(1,j,i) - GhostLeft(1,j,1,thread_nr)  )
				END DO
				...
				DO j=1,Ny
					...
					! Burger's equation
					! Uvel = weight*( Q(1,j,i) + GhostRight(1,1,j,thread_nr) )
					
					FluxInt_hor(1,j,i,thread_nr) = Uvel*weight*( Q(1,j,i) + GhostRight(1,j,1,thread_nr) ) - weight*ABS(Uvel)*( GhostRight(1,j,1,thread_nr) - Q(1,j,i) )					
				
				END DO
				
												
			END SUBROUTINE UpdateHorizontalFlux_Upwind_boundary
		
			SUBROUTINE UpdateVerticalFlux_Upwind_boundary(Q)
				DOUBLE PRECISION, DIMENSION(:,:,:), INTENT(IN) :: Q
				...
				DO i=1,Nx
					...
					! Burger's equation
					!Vvel = weight*( GhostDown(1,1,i,thread_nr) + Q(1,j,i) )
						
					FluxInt_ver(1,j,i,thread_nr) = Vvel*weight*( Q(1,j,i) + GhostDown(1,1,i,thread_nr) ) - weight*ABS(Vvel)*( Q(1,j,i) - GhostDown(1,1,i,thread_nr) )
						
					j = Ny+1
					Vvel = weight*( Q(1,j-1,i) + GhostUp(1,1,i,thread_nr) )
					FluxInt_ver(1,j,i,thread_nr) = Vvel*weight*( GhostUp(1,1,i,thread_nr) + Q(1,j-1,i) ) - weight*ABS(Vvel)*( GhostUp(1,1,i,thread_nr) - Q(1,j-1,i) )
					
					
				END DO
				
			END SUBROUTINE UpdateVerticalFlux_Upwind_boundary
	...
	END SUBROUTINE GetRHS_boundary
	...
	SUBROUTINE UpdateHorizontalFlux_inner(Qcell, max_vel)
		...
		DO j=1,Ny
			...
			Qcell_local(:,1) = Qcell(:,j,i-2) 
			Qcell_local(:,2) = Qcell(:,j,i-1)
			Qcell_local(:,3) = Qcell(:,j,i)
			Qcell_local(:,4) = Qcell(:,j,i+1)
			Qcell_local(:,5) = Qcell(:,j,i+2)
			Qcell_local(:,6) = Qcell(:,j,i+3)
			
			Fcell_local(:,1) = FluxCell_hor( :,j,i-2,thread_nr)
			Fcell_local(:,2) = FluxCell_hor( :,j,i-1,thread_nr)
			Fcell_local(:,3) = FluxCell_hor( :,j,i,  thread_nr)
	...
	END SUBROUTINE UpdateHorizontalFlux_inner
	
	SUBROUTINE UpdateHorizontalFlux_boundary(Qcell, max_vel)
		...
		DO j=1,Ny
			...
			! Two more values involving ghostcells
			DO i=1,2
					! Update local "moving window" arrays
					CALL ShiftLocalValues(Qcell_local, Fcell_local, Qcell(:,j,i+3), FluxCell_hor(:,j,i+3,thread_nr))
					CALL ReconstructInterfaceValue(Qcell_local, Fcell_local, max_vel, FluxInt_hor(:,j,i+1,thread_nr))
			...
			Qcell_local(:,4) = Qcell(:,j,i+1)
			Qcell_local(:,5) = Qcell(:,j,i+2)
			Qcell_local(:,6) = GhostRight(:,j,1,thread_nr)
			...
			Fcell_local(:,4) = FluxCell_hor(:,j,i+1,thread_nr)
			Fcell_local(:,5) = FluxCell_hor(:,j,i+2,thread_nr)
			Fcell_local(:,6) = GhostFluxRight(:,j,1,thread_nr)
	...
	END SUBROUTINE UpdateHorizontalFlux_boundary
		
	SUBROUTINE UpdateVerticalFlux_inner(Qcell, max_vel)
		...
		DO i=1,Nx
			...
			Qcell_local(:,6) = Qcell(:,j-2,i)
			Qcell_local(:,5) = Qcell(:,j-1,i)
			Qcell_local(:,4) = Qcell(:,j,  i)
			Qcell_local(:,3) = Qcell(:,j+1,i)
			Qcell_local(:,2) = Qcell(:,j+2,i)
			Qcell_local(:,1) = Qcell(:,j+3,i)
			
			Fcell_local(:,6) = FluxCell_ver(:,j-2,i,thread_nr)
			Fcell_local(:,5) = FluxCell_ver(:,j-1,i,thread_nr)
			Fcell_local(:,4) = FluxCell_ver(:,j,  i,thread_nr)
			Fcell_local(:,3) = FluxCell_ver(:,j+1,i,thread_nr)
			Fcell_local(:,2) = FluxCell_ver(:,j+2,i,thread_nr)
			Fcell_local(:,1) = FluxCell_ver(:,j+3,i,thread_nr)
	...
	END SUBROUTINE UpdateVerticalFlux_inner
	
	SUBROUTINE UpdateVerticalFlux_boundary(Qcell, max_vel)
		...
		DO i=1,Nx
			...
			Qcell_local(:,3) = Qcell(:,j+1,i)
			Qcell_local(:,2) = Qcell(:,j+2,i)
			Qcell_local(:,1) = GhostDown(:,1,i,thread_nr)
			...
			Fcell_local(:,3) = FluxCell_ver(:,j+1,i,thread_nr)
			Fcell_local(:,2) = FluxCell_ver(:,j+2,i,thread_nr)
			Fcell_local(:,1) = GhostFluxDown(:,1,i,thread_nr)
	...
	END SUBROUTINE UpdateVerticalFlux_boundary
	...
	SUBROUTINE GetFluxDivergence_inner_Upwind(RQ, dy, dx)
	...
	END SUBROUTINE GetFluxDivergence_inner_Upwind
	
	SUBROUTINE GetFluxDivergence_boundary_Upwind(RQ, dy, dx)
	...
	END SUBROUTINE GetFluxDivergence_boundary_Upwind	
	
	SUBROUTINE GetFluxDivergence_inner_WENO5(RQ, dy, dx)
	...
	END SUBROUTINE GetFluxDivergence_inner_WENO5
	
	SUBROUTINE GetFluxDivergence_boundary_WENO5(RQ, dy, dx)
	...
	END SUBROUTINE GetFluxDivergence_boundary_WENO5	
