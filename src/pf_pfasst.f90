...
contains
   ...
   subroutine pf_level_setup(pf, level_index)
      ...
      if ((lev%index < pf%nlevels) .and. (.not. allocated(lev%tauQ))) then
         if (pf%use_diag_sweeper) then
            call lev%ulevel%factory%create_array(lev%tauQ, 1, lev%index,  lev%lev_shape)
         else
      ...
      if (ierr /= 0) call pf_stop(__FILE__,__LINE__,"allocate fail")
      lev%nflags=0
      !>  initialize sdc or diag sweeper
      if (pf%use_sdc_sweeper .or. pf%use_diag_sweeper) then
      ...
      if (pf%use_diag_sweeper) then
         call lev%ulevel%factory%create_array(lev%R, 1, lev%index,  lev%lev_shape)
      else
   ...
   subroutine pf_level_destroy(pf,level_index)
      ...
      ! destroy the solution and residual arrays
      ...
      !> destroy the sweeper
      if (pf%use_sdc_sweeper .or. pf%use_diag_sweeper)  call lev%ulevel%sweeper%destroy(pf,level_index)
   ...
   subroutine pf_print_options(pf, un_opt, show_mats_opt)
      ...
      if (pf%use_sdc_sweeper) then
         ...
         if (pf%use_Sform) then
            ...
            if (pf%use_diag_sweeper) then
               write(un,*) 'Diagonal sweeper + Sform is being used - DOES NOT WORK!!!'
            end if
         else
