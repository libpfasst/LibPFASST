...
contains
   ...
   subroutine pf_predictor(pf, t0, dt, flags)
      ...
      integer                   :: diag_offset     !!  Local variable for tag-offset due to diagonal sweeper
      ...
      if (pf%debug) print*, 'DEBUG --', pf%rank_global, 'beginning predictor'
...
!    if (pf%nlevels==1) return
      ...
      if (pf%state%finest_level > 1) then
         if (pf%debug) print*,  'DEBUG --', pf%rank_global, 'do coarsen  in predictor'
      ...
      if (pf%debug) print*,  'DEBUG --', pf%rank_global, 'do burnin  in predictor'
      ...
      if (pf%state%finest_level > 1) then
         ...
         if (pf%Pipeline_pred) then
            do k = 1, c_lev%nsweeps_pred
               ...
               !  Get new initial conditions
               call pf_recv(pf, c_lev, c_lev%index*110000+diag_offset*1000+pf%rank+k, .true.)

               !  Do a sweep
               if (pf%debug) print*,  'DEBUG --',pf%rank,'sweep at pred 4,lev=',level_index
               call c_lev%ulevel%sweeper%sweep(pf, level_index, t0, dt, 1)
               !  Send forward
               call pf_send(pf, c_lev,  c_lev%index*110000+diag_offset*1000+pf%rank+1+k, .false.)
            end do ! k = 1, c_lev%nsweeps_pred-1
         else  !  Don't pipeline
            if (c_lev%nsweeps_pred .gt. 0) then
               !  Get new initial conditions
               call pf_recv(pf, c_lev, c_lev%index*110000+diag_offset*1000+pf%rank, .true.)
               !  Do a sweeps
               if (pf%debug) print*,  'DEBUG --',pf%rank,'sweep at pred 4,lev=',level_index
               call c_lev%ulevel%sweeper%sweep(pf, level_index, t0, dt, c_lev%nsweeps_pred)
               !  Send forward
               call pf_send(pf, c_lev,  c_lev%index*110000+diag_offset*1000+pf%rank+1, .false.)
            endif
