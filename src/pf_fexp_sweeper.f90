...
type, extends(pf_sweeper_t), abstract :: pf_fexp_t
	...
	procedure(pf_storeNk), deferred :: storeNk

	! REMOVE AFTER MODIFICATION IS OVER
	...
	!		stores N(y^{k+1}_j) for j = 1 ... m
        ...
        subroutine pf_expSweepSubstep(this, y_np1, j, dt, y_n, N_n)
	...
	! =================================================================================
        ! STORENK: stores the nonlinear term N_j^{[k]} j = 1 ... m that is used to compute
	! the exponential integral terms I_{n,j}. This will be called at each sweep.	
        !
	! Arguments
	!
	!   N	(in) pf_encap_t(:)
	!   	N(j) contains N_j^{[k]} = N(y_j^{[k]}) for j = 1 ... m
	! =================================================================================
	
        subroutine pf_storeNk(this, N)
            	import pf_fexp_t, pf_encap_t, pfdp
            	class(pf_fexp_t),   intent(inout) :: this
		class(pf_encap_t),  intent(in)    :: N(:)  
        end subroutine pf_storeNk
	...
	!
        ...
        subroutine pf_expResidualSubstep(this, y_np1, j, dt, y_n)
            	import pf_fexp_t, pf_encap_t, pfdp
            	class(pf_fexp_t),   intent(inout) :: this
		class(pf_encap_t),  intent(inout) :: y_np1       		
		integer,            intent(in)    :: j
	...
	! OLD METHODS [ REMOVE ONCE COMPLETE ]
    ...
    subroutine exp_initialize(this, pf,level_index)
        ...
        do i = 1, nnodes - 1
            ...
            call weights_old(this, REAL(0.0, pfdp), q, nnodes - 1, this%W(i,:,:))
        end do
    ...
    subroutine exp_sweep(this, pf, level_index, t0, dt,nsweeps, flags)
	   ...
	   call this%storeNk(this%F_old)
	      ...
	      call this%expSweepSubstep(lev%Q(j+1), j, dt, lev%Q(j), lev%F(j,1))	! compute exp(h_j L) y_{j} + \varphi_1(h_j L) * (F^{k+1}_j - F^{k}_j) + I_j
	...
	call this%expResidualSubstep(fintsdc(i), i, dt, qSDC(1))         
    ...
    subroutine weights_old(this, z, x, m, W)
