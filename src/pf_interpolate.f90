...
contains
  ...
  subroutine interpolate_time_space(pf, t0, dt, level_index, F_INTERP, flags)
    ...
    do m = 1, c_lev_ptr%nnodes
       call c_delta(m)%setval(0.0_pfdp, which)
       call cf_delta(m)%setval(0.0_pfdp, which)
    end do

    !>  interpolate coarse level correction in space only
    do m = 1, c_lev_ptr%nnodes
       call c_delta(m)%copy(c_lev_ptr%Q(m), which)
       call c_delta(m)%axpy(-1.0_pfdp, c_lev_ptr%pQ(m), which)
       call f_lev_ptr%ulevel%interpolate(f_lev_ptr,c_lev_ptr, cf_delta(m), c_delta(m), c_times(m), which)
    end do

    !> interpolate corrections in time
    call pf_apply_mat(f_lev_ptr%Q, 1.0_pfdp, f_lev_ptr%tmat, cf_delta, .false., which)
    ...
    if (F_INTERP) then         !  Interpolating F
      do p = 1,size(c_lev_ptr%F(1,:))
          ...
          do m = 1, c_lev_ptr%nnodes
            call c_delta(m)%copy(c_lev_ptr%F(m,p), which)
            call c_delta(m)%axpy(-1.0_pfdp, c_lev_ptr%pF(m,p), which)
            call f_lev_ptr%ulevel%interpolate(f_lev_ptr, c_lev_ptr, cf_delta(m), c_delta(m), c_times(m), which)
         end do

         ! interpolate corrections  in time
          call pf_apply_mat(f_lev_ptr%F(:,p), 1.0_pfdp, f_lev_ptr%tmat, cf_delta, .false., which)

       end do !  Loop on npieces
    else    ! recompute function values
       call f_lev_ptr%ulevel%sweeper%evaluate_all(f_lev_ptr, f_times, which, step)
    end if  !  Feval
