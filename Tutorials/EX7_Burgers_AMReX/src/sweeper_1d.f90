...
contains
  ...
  subroutine advect_1d (lo, hi, y, f, plo, phi, dx)
    ...
    real(amrex_real), intent(inout) :: f(plo(1):phi(1))
    real(amrex_real), intent(in) :: dx(1)
    ! local
    integer :: ix
    real(amrex_real) :: dxinv
    real(amrex_real) :: fx(lo(1):hi(1)+1)
    ...
    ! x-fluxes
    do ix=lo(1),hi(1)+1
      fx(ix) = ( y(ix-1) + y(ix) ) * 0.5_pfdp    ! upstream
      !fx(ix) = (7.0_pfdp*(y(ix) + y(ix-1)) - y(ix-2) - y(ix+1)) / 12.0_pfdp
    end do

    ! eval f
    do ix=lo(1),hi(1)      
      f(ix) = -dxinv * y(ix) * (fx(ix+1) - fx(ix))
    end do

  end subroutine advect_1d

  subroutine diff_1d (lo, hi, y, f, plo, phi, dx)
    ! evaluate RHS for single patch
    use amrex_fort_module,  only : amrex_real
    use probin,             only: nu
    ! in- / outputs
    ...
    ! x-fluxes
    do ix=lo(1),hi(1)+1
      fx(ix) = ( y(ix) - y(ix-1) ) * dxinv
      !fx(ix) = (15.0_pfdp*(y(ix) - y(ix-1)) + y(ix-2) - y(ix+1)) * dxinv / 12.0_pfdp 
    end do

    ! eval f - value is divergence of flux
    do ix=lo(1),hi(1)      
      f(ix) = nu * (fx(ix+1) - fx(ix)) * dxinv
    end do
  ...
  subroutine f_eval(this, y, t, level_index, f,piece)
    use probin, only: splitting, Ndim
    ...
    if (piece .eq. 1) then  !  Explicit solve of advection term
      ...
      do while (mfi%next())
        ...
        ! eval advect_1d
        call advect_1d(bx%lo, bx%hi, y_arr, f_arr, plo, phi, this%y_encap%geom%dx)
      end do
      ...
      do while (mfi%next())
        ...
        ! eval diff_1d
        call diff_1d(bx%lo, bx%hi, y_arr, f_arr, plo, phi, this%y_encap%geom%dx)
      end do
  ...
  ! F_COMP: solve for y and return f2 also.
