...
module pf_my_sweeper
   ...
   type, extends(pf_imex_diag_sweeper_t) :: my_sweeper_t
      integer ::     nx, ny
      ...
      type(amrex_abeclaplacian), allocatable :: abeclap
      type(amrex_multigrid),       allocatable :: mlmg
...
contains
   ...
   subroutine advect_2d (lo, hi, y, f, plo, phi, dx)
      ...
      use probin,             only: vx, vy        ! advective velocities
      ! in- / outputs
      ...
      do iy=lo(2),hi(2)
         do ix=lo(1),hi(1)
            f(ix,iy) = -vx * dxinv * (fx(ix+1,iy  )-fx(ix,iy)) &
               -vy * dyinv * (fy(ix  ,iy+1)-fy(ix,iy))
         end do
      end do
   ...
   subroutine initialize(this, pf,level_index)
      ...
      call this%imex_diag_initialize(pf,level_index)
      ! set IMEX options
      if (splitting .eq. 0 ) then     ! do implicit only
         this%implicit=.TRUE.
         this%explicit=.FALSE.
         if (ioProc) print *, 'sweeper_2d.f90: fully implicit detected'
      elseif (splitting .eq. 1 ) then ! IMEX
         this%implicit=.TRUE.
         this%explicit=.TRUE.
         if (ioProc) print *, 'sweeper_2d.f90: IMEX detected'
      elseif (splitting .eq. 2 ) then ! do explicit only
         this%implicit=.FALSE.
         this%explicit=.TRUE.
         if (ioProc) print *, 'sweeper_2d.f90: fully explicit detected'
   ...
   subroutine destroy(this,pf,level_index)
      ...
      !> Destroy AMReX MLMG & Laplacian
         if (allocated(this%abeclap)) then
            call amrex_abeclaplacian_destroy(this%abeclap)
            deallocate(this%abeclap)
         end if
         if (allocated(this%mlmg)) then
            call amrex_multigrid_destroy(this%mlmg)
            deallocate(this%mlmg)
         end if

      !>  Call base sweeper destroy
      call this%imex_diag_destroy(pf,level_index)
   ...
   subroutine f_eval(this, y, t, level_index, f,piece)
      ...
      if (piece .eq. 1) then  !  Explicit solve of advection term
         ...
         call this%y_encap%mfab%fill_boundary(this%y_encap%geom)
   ...
   subroutine f_comp(this, y, t, dtq, rhs, level_index, f,piece)
      ...
      ! Avoid local amrex_multifab copies; use array constructors below
      ...
      if (piece == 1) then        ! implicit advection
         ...
         ! Call MLMG directly with single-element arrays (array constructors)
...
! boundary conditions used in AMReX (BCType defined in path_to_amrex/Src/Base/AMReX_BC_TYPES.H)
 !    ---------------------------------------------------------------------
