...
contains
   ...
   subroutine run_pfasst()
      ...
      type(pf_comm_t)       :: comm           !<  pfasst block communicator
      type(pf_comm_t)       :: commD          !<  diagonal sweeper communicator
      ...
      !> mpi parameters
      integer :: space_comm, time_comm, diag_comm, space_color, time_color, diag_color, err
      integer :: sSize, io
      real    :: startT, endT
      ...
      if (ndiag > 1) then
         if (gRank .eq. 0) print *, ' '
         if (gRank .eq. 0) print *, 'Using Diagonal Sweeper'
         if (gRank .eq. 0) print *, 'ndiag:', ndiag, 'ntime:', ntime, 'nspace:', nspace 
         call pf_mpi_orthosplit(nspace, ntime, space_comm, time_comm, space_color, time_color, MPI_COMM_WORLD, & 
                                 ndiag, diag_comm, diag_color)
      else 
         call pf_mpi_orthosplit(nspace, ntime, space_comm, time_comm, space_color, time_color, MPI_COMM_WORLD)
      end if
      ! get space-size for sanity check of number of boxes
      call mpi_comm_size(space_comm, sSize, err)

      !>  create the pfasst structure
      call pf_mpi_create(comm, time_comm)
      if (gRank .eq. 0) print *,'main.f90:creating pf'
      if (ndiag > 1) then
         call pf_mpi_create(commD, diag_comm)
         call pf_pfasst_create(pf, comm, diag_comm=commD, fname=pf_fname)   
      else
         call pf_pfasst_create(pf, comm, fname=pf_fname)
      end if
      if (gRank .eq. 0) print *,'main.f90: pf created'
      ...
      do l = 1, pf%nlevels
         ...
         if (gRank .eq. 0) print *, 'box_per_proc: ', box_per_proc
         if (gRank .eq. 0) print *, 'nghost: ', grid_size(Ndim+1)
         if (gRank .eq. 0) print *, 'mpibuflen: ', mpibuflen
         if (gRank .eq. 0) print *, 'Level: ', l
         !if (gRank .eq. 0) print *, 'BoxArray: '
         !if (gRank .eq. 0) call amrex_print(ba(l))
         !if (gRank .eq. 0) print *, 'Distribution Map: '
         !if (gRank .eq. 0) call amrex_print(dm(l))
         if (gRank .eq. 0 .and. Ndim .eq. 1) then
            if (gRank .eq. 0) print *,'grid_size_amrex: ', grid_size_amrex, ' [nx,ncomp,nghost]'
         else if (gRank .eq. 0 .and. Ndim .eq. 2) then
            if (gRank .eq. 0) print *,'grid_size_amrex: ', grid_size_amrex, ' [nx,ny,ncomp,nghost]'
         else if (gRank .eq. 0 .and. Ndim .eq. 3) then
      ...
      call pf_add_hook(pf, -1, PF_PRE_PREDICTOR, echo_error)
...
1000  format (A5,I2,A20,F10.5,A4)
      ...
      !> print final
      if (gRank == 0) print *, "Writing AMReX output files!"
      call sleep(1)        ! sometimes write below fails, short pause somehow fixes this
