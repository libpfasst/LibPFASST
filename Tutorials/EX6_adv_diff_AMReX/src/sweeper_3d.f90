...
contains
   ...
   subroutine advect_3d (lo, hi, y, f, plo, phi, dx)
      ...
      use probin,             only: vx, vy, vz        ! advective velocities
      ! in- / outputs   
    ...
    do iz=lo(3),hi(3)
       do iy=lo(2),hi(2)
          do ix=lo(1),hi(1)
             f(ix,iy,iz) = -vx * dxinv * (fx(ix+1,iy  ,iz  ) - fx(ix,iy,iz)) &
                         -vy * dyinv * (fy(ix  ,iy+1,iz  ) - fy(ix,iy,iz)) &
                         -vz * dzinv * (fz(ix  ,iy  ,iz+1) - fz(ix,iy,iz))
          end do
       end do
    end do
  ...
  !
   ...
   subroutine initialize(this, pf,level_index)
      ...
      if (splitting .eq. 0 ) then     ! do implicit only
         this%implicit=.TRUE.
         this%explicit=.FALSE.
         if (ioProc) print *, 'sweeper_3d.f90: fully implicit detected'
      elseif (splitting .eq. 1 ) then ! IMEX
         this%implicit=.TRUE.
         this%explicit=.TRUE.
         if (ioProc) print *, 'sweeper_3d.f90: IMEX detected'
      elseif (splitting .eq. 2 ) then ! do explicit only 
         this%implicit=.FALSE.
         this%explicit=.TRUE.
         if (ioProc) print *, 'sweeper_3d.f90: fully explicit detected'
