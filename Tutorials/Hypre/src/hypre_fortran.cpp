...
extern "C"
{
   ...
   void HypreSolverInit(HypreSolver **hypre_solver, int pfasst_level_index, int num_grid_points, int comm_color, int space_dim, int max_iter, int max_levels)
   {
      ...
      *hypre_solver = new HypreSolver(newcomm, space_dim, max_iter, max_levels);
      (*hypre_solver)->SetupMatrix(num_grid_points, level_index);
      if (level_index == 0){
   ...
   {
      int level_index = PfasstToHypreLevelIndex(pfasst_level_index, x->GetNumLevels());
